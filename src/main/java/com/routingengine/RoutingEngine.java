...
public class RoutingEngine
{
    ...
    final Map<Type, List<Agent>> availableAgents;
    ...
    public RoutingEngine()
    {
        ...
        availableAgents = initAvailableAgents();
    ...
    private Map<Type, List<Agent>> initAvailableAgents() {
        Map<Type, List<Agent>> availableAgents = new HashMap<Type, List<Agent>>();
        Agent[] agents = getAgents();
        for (Type type : SupportRequest.Type.values()) {
            List<Agent> agentList = new ArrayList<Agent>();
            availableAgents.put(type, agentList);
        }
        ...
        availableAgents = Collections.synchronizedMap(availableAgents);
        return availableAgents;
    }

    public Runnable[] getWorkers() {
        Runnable[] runnables = new Runnable[Type.values().length];
        for (int i = 0 ; i < runnables.length ; i++) {
            Type type = SupportRequest.Type.of(i);
            RequestQueue requestQueue = requestQueueManager.getQueue(type);
